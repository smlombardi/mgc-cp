  ...
  $( window.document ).bind('mobileinit', function(){
    ...
    if ($.support.splitview) {
      ...
      $(function() {
		...
		firstPageMainSelector = 'div:jqmData(id="main") > div:jqmData(role="page"):first';
        if( !$.mobile.hashListeningEnabled || !$.mobile.path.stripHash( location.hash ) ){
          var $container=$('div:jqmData(id="main")');
          $.mobile.firstPage = firstPageMain;
          $.mobile.pageContainer = $container;
          $(window).trigger('pagecontainercreate');
	 // $.mobile.changePage() is deprecated in jqm 1.4 and replaced with $.mobile.pageContainer.pagecontainer ("change", "#page_id");, try it. –  Omar Nov 21 '13 at 
          //$.mobile.changePage(firstPageMain, {transition:'none', changeHash:false, pageContainer:$container});
	  $.mobile.pageContainer.pagecontainer ("change", firstPageMain,{transition:'none', changeHash:false, pageContainer:$container});
...
//----------------------------------------------------------------------------------
	  ...
	  $.mobile.pageContainer.pagecontainer ("change", url,{
        ...
        $(document).bind( "click", function(event) {
	     ...
	     $.mobile.pageContainer.pagecontainer ("change", href,{fromPage:from, transition:'fade', reverse:reverseVal, changeHash:false, pageContainer:$targetContainer, reloadPage:isRefresh, role:role, link:$link});
            }
	    ...
	    $.mobile.pageContainer.pagecontainer ("change", href,{fromPage:from, transition:transitionVal, reverse:reverseVal, changeHash:hashChange, pageContainer:$currPanel, role: role, link: $link});
        ...
        $.mobile._handleHashChange = function( hash ) {
	      ...
	      $.mobile.pageContainer.pagecontainer ("change", $menuPanelFirstPage,{transition:'none', reverse:true, changeHash:false, fromHashChange:false, pageContainer:$menuPanel});
              $.mobile.activePage=undefined;
            }
            $.mobile.activePage=$mainPanelActivePage.length? $mainPanelActivePage : undefined;
            $.mobile.pageContainer.pagecontainer ("change",to, changePageOptions );
          ...
          //there's no hash, go to the first page in the main panel.
            ...
            $.mobile.pageContainer.pagecontainer ("change", $mainPanelFirstPage, changePageOptions ); 
          }
        };
      ...
      $(document).on('pagechange.context','div:jqmData(role="panel")', function(){
        ...
        if($.type(contextSelector) === 'object') {
          ...
          if(($targetPanelActivePage.jqmData('url') == contextSelector.url && contextSelector.refresh)||(!contextSelector.refresh && $targetPanelActivePage.jqmData('url') != contextSelector.url)){    
              $.mobile.pageContainer.pagecontainer ("change",contextSelector.url, options={transition:'fade', changeHash:false, pageContainer:$targetContainer, reloadPage:isRefresh});
          }
        }
